...
function PaymentMethodAutocomplete({
  ...
  const formatDisplayLabel = useCallback((paymentMethod: SiigoPaymentMethodOption | null) => {
  ...
  const filteredOptions = useMemo(() => {
    ...
    if (!query || (value && formatPaymentMethodOptionLabel(value).toLowerCase() === query)) {
      return options
    }
  ...
  const selectOption = useCallback(
    (paymentMethod: SiigoPaymentMethodOption) => {
      ...
      setInputValue(formatDisplayLabel(paymentMethod))
      setIsOpen(false)
    },
      ...
      {isOpen && !disabled && (
        ...
        >
          {filteredOptions.length === 0 ? (
            ...
            filteredOptions.map((paymentMethod, index) => (
              <li
                key={paymentMethod.id}
                role="option"
                aria-selected={value?.id === paymentMethod.id}
                ...
                onClick={() => selectOption(paymentMethod)}
              >
                {formatPaymentMethodOptionLabel(paymentMethod)}
              </li>
            ))
          )}
        </ul>
